...
function symfony(config) {
    ...
    return [
        resolveSymfonyPlugin(pluginConfig),
        ...resolveFullReloadConfig(pluginConfig)
    ];
}
function resolveSymfonyPlugin(pluginConfig) {
    ...
    return {
        ...
        config: (config, { command, mode }) => {
            ...
            return {
                ...
                server: {
                    origin: userConfig.server?.origin ?? "__symfony_vite_placeholder__",
        ...
        transform(code) {
            if (resolvedConfig.command === "serve") {
                code = code.replace(/__symfony_vite_placeholder__/g, viteDevServerUrl);
...
function resolvePluginConfig(config) {
    if (typeof config === "undefined") {
        throw new Error("symfony-vite-plugin: missing configuration.");
    }
    if (typeof config === "string" || Array.isArray(config)) {
        config = { input: config, ssr: config };
    }
    if (typeof config.input === "undefined") {
        throw new Error('symfony-vite-plugin: missing configuration for "input".');
    }
    if (typeof config.publicDirectory === "string") {
        config.publicDirectory = config.publicDirectory.trim().replace(/^\/+/, "");
        if (config.publicDirectory === "") {
            throw new Error("symfony-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'.");
        }
    }
    if (typeof config.buildDirectory === "string") {
        config.buildDirectory = config.buildDirectory.trim().replace(/^\/+/, "").replace(/\/+$/, "");
        if (config.buildDirectory === "") {
            throw new Error("symfony-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'.");
        }
    }
...
function resolveFullReloadConfig({ refresh: config }) {
    ...
    return config.flatMap((c) => {
        ...
        plugin.__symfony_plugin_config = c;
        return plugin;
    });
}
...
function noExternalInertiaHelpers(config) {
    ...
    const pluginNoExternal = ["symfony-vite-plugin"];
